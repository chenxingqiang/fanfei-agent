 ...
 * the fanfeiChatbotConfig should be defined in the html file before this script is included
 * the fanfeiChatbotConfig should contain the token of the chatbot
...
(function () {
  ...
  const configKey = "fanfeiChatbotConfig";
  const buttonId = "fanfei-chatbot-bubble-button";
  const iframeId = "fanfei-chatbot-bubble-window";
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with fanfei-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with fanfei-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    const baseUrl =
      config.baseUrl || `https://${config.isDev ? "dev." : ""}ufanfei.app`;
    ...
    function createIframe() {
      ...
      iframe.title = "fanfei chatbot bubble window";
      iframe.id = iframeId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'fanfei-chatbot-iframe-ready') {
        targetIframe.contentWindow?.postMessage(
          {
            type: 'fanfei-chatbot-config',
            payload: {
      ...
      if (event.data.type === 'fanfei-chatbot-expand-change') {
        toggleExpand();
      }
    });
