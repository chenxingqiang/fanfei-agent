const translation = {
  ...
  plansCommon: {
    ...
    apiRateLimitTooltip: 'API 请求频率限制涵盖所有通过 Fanfei API 发起的调用，例如文本生成、聊天对话、工作流执行和文档处理等。',
    ...
    messageRequest: {
      ...
      tooltip: '消息额度旨在帮助您便捷地试用 Fanfei 中的各类 OpenAI 模型。不同模型会消耗不同额度。额度用尽后，您可以切换为使用自己的 OpenAI API 密钥。',
    },
    annotatedResponse: {
      title: '{{count,number}} 个标注回复数',
      tooltip: '标注回复功能通过人工编辑标注为应用提供了可定制的高质量问答回复能力。',
    },
    ragAPIRequestTooltip: '指单独调用 Fanfei 知识库数据处理能力的 API。',
