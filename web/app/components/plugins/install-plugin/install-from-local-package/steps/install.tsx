...
const Installed: FC<Props> = ({
  ...
  const isFanfeiVersionCompatible = useMemo(() => {
    if (!langeniusVersionInfo.current_version)
      return true
    return gte(langeniusVersionInfo.current_version, payload.meta.minimum_fanfei_version ?? '0.0.0')
  }, [langeniusVersionInfo.current_version, payload.meta.minimum_fanfei_version])
    ...
    <>
      <div className='flex flex-col items-start justify-center gap-4 self-stretch px-6 py-3'>
        <div className='system-md-regular text-text-secondary'>
          ...
          {!isFanfeiVersionCompatible && (
            <p className='system-md-regular flex items-center gap-1 text-text-warning'>
              {t('plugin.fanfeiVersionNotCompatible', { minimalFanfeiVersion: payload.meta.minimum_fanfei_version })}
            </p>
          )}
        </div>
