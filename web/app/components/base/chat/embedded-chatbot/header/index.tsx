...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback((event: MessageEvent) => {
    ...
    if (!currentParentOrigin && event.data.type === 'fanfei-chatbot-config') {
      currentParentOrigin = event.origin
      setParentOrigin(event.origin)
    }
    if (event.origin !== currentParentOrigin)
      return
    if (event.data.type === 'fanfei-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({ type: 'fanfei-chatbot-iframe-ready' }, '*')
  ...
  const handleToggleExpand = useCallback(() => {
    ...
    window.parent.postMessage({
      type: 'fanfei-chatbot-expand-change',
    }, parentOrigin)
